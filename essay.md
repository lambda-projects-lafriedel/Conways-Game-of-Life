...
This week I built simulation of Conway's Game of Life and would love to share how I approached building it! My hope is that this guide will help someone think how to plan out their own version of the game. Check out my [deployed project](https://conway-game-of-life-4a4e2.firebaseapp.com/).
...
So, initially, this was a big project to tackle. *"How do I create this?"* Honestly, I was a bit overwhelmed at first. But working through the project using [Polya's Four Steps to Problem-Solving](<http://www.ms.uky.edu/~lee/ma310sp15/polya.pdf>), I was able to break it down into manageable chunks.

Before I began, I made sure I understood what the goal was, and that I understood the rules of the Game. I conducted some research to help clarify some questions by reading some Wikipedia and Medium articles about the Game.

Once I felt comfortable with that, I began to formulate my plan of attack.

In a high-level view, here are the items I knew needed to be built:

* The data structure of the actual game that holds information about the grid and each cell
* The visual depiction of the game, which would be a graph that contains cells, and you could see whether the cells were alive or dead
* The functionality that would change the data structure for each generation
* The functionality that would change the visual depiction of the game
* Controls displayed on the page (buttons, text inputs, etc.) that would allow customization

I'll go through an overview of each bullet-point above and further break down each item.

## Creating the game's basic data structure

The game needs a way to store information about the grid and it's current state. It's best to separate this out from the visual depiction of the game, so that your code stays organized.
...
* A vanilla Javascript `Cell` class, which would hold the x and y coordinate of the cell, as well as it's current state (alive or dead)
...
* The ability to click on a cell that, when clicked, changed the fill color of a cell (from white to black, signifying the cell is alive, or from black to white, signifying the cell is dead)
...
* `applyRulesAndMigrateToBuffer` which, after receiving the number of alive neighbors, applies the rules of Conway's Game of Life to each cell. After that is determined, the buffer receives the new state of each cell to start creating the next generation.
...
* `clearGridAndBuffer` which would set all the cells' states to dead
...
First we need some basic controls displayed in the React component:

* A start button
* A stop button
* A clear button

The React component would also need these functions:
...
* `tick` which is the driving force behind the constant animation. It calls the data structure's `updateGridAndBuffer` method and the component's `drawNextGen` function, and then continually calls `requestAnimationFrame` and repeats itself as long as the user has not pressed the "Stop" button
* `stopGame` which is called when the user presses the "Stop" button, and pauses the game
* `clearCanvas` which calls the data structure's `clearGridAndBuffer` method and also calls `drawNextGen` to update all the cells to appear as dead. This is called when the user presses the "Clear" button after the game is stopped

Once these are all properly written and implemented, the game will run! Exciting! But I wanted to add some additional functionality to encourage further experimentation with the game.
...
* A "Step 1 Gen" button to allow you to step one generation at a time manually
* A text input and submit button that allows you to change the overall size of the grid (prior to initiating the game animation)
* A text input and submit button that allows you to change the size of each cell (prior to initiating the game animation)

The "Gimme random!" button required a couple additonal functions:

* `generateRandomFirstGen` method inside the `Grid` class, that would utilize `Math.random()` to decide whether a cell was alive or dead. Since `Math.random()` returns a number between 0 and 1 (exclusive of 0 and 1), I decided I wanted a 25% chance that a cell would be alive. So if `Math.random()` returned a number less than `0.25` a cell would be alive.
...
The "Step 1 Gen" button also required one additional function inside the React component:
